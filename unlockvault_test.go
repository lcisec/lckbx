...
var (
	unlockedVaultDB   = "unlocked_test.db"
	unlockedVaultUser = "uv_user"
	vaultNoteName     = "A Note"
	vaultNoteData1    = []byte("Original note.")
	vaultNoteData2    = []byte("Updated note.")
)

// End-to-end test for an UnlockedVault
...
func TestUnlockedVault(t *testing.T) {
	...
	// 1.b Create a new vault with the given store.
	...
	// 2.  Login as the user, add an item, lock the vault
	// 2.a Login as the user
	uv, err := vault.Login(unlockedVaultUser, vaultGoodPassword)
	if err != nil {
		t.Fatalf("Expected no error, received %v", err)
	}
	...
	n := NewNoteItem()
	iid := n.ItemId
	n.Name = vaultNoteName
	n.Data = vaultNoteData1

	// 2.c Add the NoteItem to the database.
	err = uv.AddNoteItem(n)
	if err != nil {
		t.Fatalf("Expected no error, received %v", err)
	}
	...
	key, _ := uv.keyset.GetNewItemKey(n.ItemId)
	uv.crypt.ChangeKey(key[:])

	// 3.c Read the item from the store and make sure it is the same as what
	//     we saved.
	n2, err := NewNoteItemFromStore(uv.store, uv.crypt, iid)
	if err != nil {
		t.Fatalf("Expected no error, received %v", err)
	}
	...
	n2.Data = vaultNoteData2
	err = uv.UpdateNoteItem(n2)
	if err != nil {
		t.Fatalf("Expected no error, received: %v", err)
	}
	...
	//     we saved.
	n3, err := NewNoteItemFromStore(uv.store, uv.crypt, iid)
	if err != nil {
		t.Fatalf("Expected no error, received %v", err)
	}
	...
	// 5.  Change password and ensure we can still read the item
	err = vault.ChangePassword(unlockedVaultUser, vaultGoodPassword, vaultBadPassword)
	if err != nil {
		t.Fatalf("Expected no error, received %v", err)
	}

	// 5.a Login as the user
	uv, err = vault.Login(unlockedVaultUser, vaultBadPassword)
	if err != nil {
		t.Fatalf("Expected no error, received %v", err)
	}

	// 5.b Ensure we have two keys in our keyset now.
	if len(uv.keyset.Keys) != 2 {
		t.Fatalf("Expected two keys in our keyset, found %d", len(uv.keyset.Keys))
	}
	...
	//     key.
	imd, _ := uv.metadata.GetItem(n.ItemId)
	key, _ = uv.keyset.GetItemKey(imd.KeyVersion, n.ItemId)
	uv.crypt.ChangeKey(key[:])

	// 5.d Read the item from the store and make sure it is the same as what
	//     we saved.
	n4, err := NewNoteItemFromStore(uv.store, uv.crypt, iid)
	if err != nil {
		t.Fatalf("Expected no error, received %v", err)
	}
	...
	if len(uv.keyset.Keys) != 1 {
		t.Fatalf("Expected one key in our keyset, found %d", len(uv.keyset.Keys))
	}
	...
	//     we saved.
	n5, err := NewNoteItemFromStore(uv.store, uv.crypt, iid)
	if err != nil {
		t.Fatalf("Expected no error, received %v", err)
	}
